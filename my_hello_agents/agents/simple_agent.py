...
from my_hello_agents.tool.registry import ToolRegistry
...
class MySimpleAgent(SimpleAgent):
    ...
    def _get_enhanced_system_prompt(self) -> str:
        ...
        tools_section = "\n\n## 可用工具\n"
        tools_section += "你可以使用以下工具来帮助回答问题, 一次只能调用一个工具:\n"
    ...
    def _run_with_tools(self, messages: list, **kwargs) -> str:
        """支持工具调用的运行逻辑"""
        final_response = ""
        # 调用LLM
        response = self.llm.think(messages, **kwargs)
        ...
        # 检查是否有工具调用
        call = self._parse_tool_calls(response)

        if call:
            print(f"🔧 检测到 {len(call)} 个工具调用")
    ...
    def _parse_tool_calls(self, text: str) -> dict:
        """解析文本中的工具调用"""
        pattern = r'\[TOOL_CALL:([^:]+):([^\]]+)\]'
